...
def dump_spec(spec: ModelSpec, path: Path) -> None:
    """Write the spec, replacing `path` only once it is complete.

    `profile` writes this file repeatedly as it works, so an interrupt has
    to be able to land during a write. Building the replacement beside the
    target and moving it into place means the reader either sees the
    previous spec or the new one, never half of either.
    """
    ...
    try:
        with tmp.open("w", encoding="utf-8") as fh:
            fh.write(
                ...
                "# Do not hand-edit: put changes in your overlay file "
                "instead,\n"
