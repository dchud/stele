...
#: First line of every emitted file, and the marker that says stele wrote it.
_HEADER = "Generated by stele. Do not edit."
...
def _prune_stale(outdir: Path, keeping: set[str]) -> list[str]:
    """Drop modules a previous run wrote and this one does not.

    A table that goes away upstream would otherwise leave its module behind,
    no longer imported by ``__init__`` but still importable and still
    readable as though it described something. Only files carrying the
    generated header are removed, and only in a directory that already holds
    a generated package, so pointing ``--out`` at the wrong directory costs
    nothing.
    """
    init = outdir / "__init__.py"
    if not init.is_file() or _HEADER not in init.read_text(encoding="utf-8"):
        return []
    removed = []
    for path in sorted(outdir.glob("*.py")):
        if path.name in keeping:
            continue
        if _HEADER not in path.read_text(encoding="utf-8")[:200]:
            continue
        path.unlink()
...
def generate(
    ...
    gen.report.removed_modules = _prune_stale(
        outdir,
        {"__init__.py", "_schemas.py"}
        | {f"{m.module_name}.py" for m in modules},
    )
